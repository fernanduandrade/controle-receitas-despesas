...
const TableInfo = () => {
  ...
  async function deleteRegister(id) {
    try {
      const updatedData = { ...financial };
      api.delete(`budget/${id}/`).then(() => {
        }).then(() => {
          setFinancial(updatedData);
      })
    } catch(error) {
      console.log(error);
    }
      ...
      <tbody>
        {financial.map(element => {
          return <tr key={element.id} className='text-center'>
            <td>{formartDate(element.date)}</td>
            ...
            <td>
              <Button onClick={() => deleteRegister(element.id)} variant="primary-outline"><FontAwesomeIcon icon={faTrash} color="#C43E36"/></Button>
            </td>
          </tr>
      })}
      </tbody>
